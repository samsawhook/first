...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey) {
      console.error("[access-request] RESEND_API_KEY missing in env");
    ...
    // 1) Operator notification — this is the critical one. If it fails, fail the request.
    const opResult = await resend.emails.send({
    ...
    if (opResult.error) {
      console.error("[access-request] operator email failed:", opResult.error);
      return NextResponse.json(
        { error: `Resend rejected operator email: ${opResult.error.message ?? opResult.error.name ?? "unknown"}` },
        { status: 502 },
      );
    }

    // 2) Requester confirmation — best-effort. Log and continue if this fails so the
    //    user still sees a successful submission.
    ...
    if (confResult.error) {
      console.warn("[access-request] confirmation email failed:", confResult.error);
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    console.error("[access-request] uncaught error:", err);
