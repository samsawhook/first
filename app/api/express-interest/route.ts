...
export async function POST(req: Request) {
  ...
  const { company, side, name, email, phone } = body;
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #0f172a;">
      <div style="background: #0D1421; color: #e2e8f0; padding: 24px 32px; border-radius: 8px 8px 0 0;">
        <h1 style="margin: 0; font-size: 20px; font-weight: 600;">Indication of Interest${sideLabel}</h1>
      ...
      <div style="background: #f8fafc; padding: 24px 32px; border: 1px solid #e2e8f0; border-top: none;">
        <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
          <tr><td style="padding: 8px 0; color: #64748b; width: 160px;">Company / Deal</td><td style="padding: 8px 0; font-weight: 600;">${company}${sideLabel}</td></tr>
  ...
  try {
    await resend.emails.send({
      ...
      reply_to: email,
      subject: `[IOI] ${company}${sideLabel} — ${name}`,
      html,
    });
    ...
    await resend.emails.send({
      ...
      to: [email],
      subject: `Your indication of interest for ${company} has been received`,
      html: `
        <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; color: #0f172a; padding: 32px;">
          ...
          <p style="color: #64748b; font-size: 14px; line-height: 1.6;">
            Your non-binding IOI for <strong>${company}</strong> has been logged.
            The nth Venture team will follow up within 2 business days.
          </p>
